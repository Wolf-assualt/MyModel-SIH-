...
/** A jest.fn-style assertion helper built on top of vitest. */
 ...
 * the InvestigationProvider. Each method is a vi.fn() so tests can assert
 * calls and override return values.
 */
export function createMockApiService() {
  const calls: Record<string, MockCall[]> = {};

  function track(methodName: string, fn: (...args: unknown[]) => unknown) {
    return (...args: unknown[]) => {
  ...
  const mock = {
    // ── Upload + Scan ──
    uploadAndScanDataset: track('uploadAndScanDataset', (_file: File) => Promise.resolve<ScanSession>({
    ...
    getScanSession: track('getScanSession', (_scanId: string) => Promise.resolve<ScanSession>({
      scan_id: 'scan-mock-001',
      batch_id: 'batch-mock-001',
      created_at: '2026-01-01T00:00:00Z',
      status: 'COMPLETED',
    ...
    quarantineDatasetImage: track('quarantineDatasetImage', (_batch: string, _sample: string) =>
      Promise.resolve({ sample_id: 'mock', file_name: '', sha256_hash: '', result: 'POISONED / ALTERED', integrity_status: 'FAIL', trust_status: 'REVOKED', anomaly_score: null, evidence: [], action: 'QUARANTINE', preview_data_url: null, quarantined: true })
    ),

    // ── Ledger ──
    verifyLedger: track('verifyLedger', () => Promise.resolve<LedgerVerification>({
      valid: true,
      events_checked: 10,
    ...
    fetchLedgerEvents: track('fetchLedgerEvents', () => Promise.resolve<LedgerEventRecord[]>([])),

    recordAnalystDecision: track('recordAnalystDecision', (
      _entityId: string, _decision: string, _actor: string, _opts: Record<string, unknown> = {}
    ) => Promise.resolve<LedgerEventRecord>({
      sequence: 1,
      timestamp: '2026-01-01T00:00:00Z',
      event_type: 'analyst_decision',
      actor: _actor,
      scan_id: typeof _opts['scanId'] === 'string' ? _opts['scanId'] : null,
      entity_id: _entityId,
      payload_hash: '0000',
      previous_hash: '0000',
      current_hash: '0000',
    ...
    // ── Graph ──
    fetchGraphExport: track('fetchGraphExport', () => Promise.resolve<BackendGraphExport>({
      nodes: [
        { id: 'n1', node_type: 'DATASET_BATCH', label: 'batch-001', properties: { status: 'warning' } },
      ],
      edges: [],
    ...
    fetchOverview: track('fetchOverview', () => Promise.resolve<SystemHealthOverview>({
      total_datasets: 5, total_models: 3, total_inferences: 1024, total_reports: 7,
      quarantined_assets: 1, under_review_assets: 2, accepted_assets: 2,
      active_threats_count: 0, chain_head_hash: 'abc123', system_integrity_status: 'OPERATIONAL',
    })),

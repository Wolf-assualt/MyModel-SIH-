    ...
    <section style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '24px' }}>
        {imageResults.map(image => (
          <article key={image.sample_id} style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: '12px', overflow: 'hidden' }}>
            ...
            <div style={{ padding: '1rem', display: 'flex', flexDirection: 'column', gap: '0.55rem' }}>
              ...
              <div style={{ color: resultColor(image.result), fontWeight: 500 }}>{image.result}</div>
              ...
              <div style={{ fontSize: '11px', color: 'var(--text-secondary)', overflowWrap: 'anywhere' }} className="font-mono">Trust fingerprint<br />SHA-256: {image.sha256_hash}</div>
              ...
              {image.quarantined ? (
                <div style={{ color: 'var(--critical-text)', fontWeight: 500 }}><LockKeyhole size={15} strokeWidth={1.5} style={{ verticalAlign: 'middle' }} /> QUARANTINED<br /><span style={{ fontWeight: 400 }}>Status: BLOCKED | Trust: REVOKED | Action: Excluded from inference</span></div>
              ) : image.action === 'QUARANTINE' ? (
                <Button variant="danger" size="sm" onClick={() => handleQuarantine(image)} disabled={busy === image.sample_id} icon={<LockKeyhole size={14} strokeWidth={1.5} />}>{busy === image.sample_id ? 'Quarantining…' : 'Quarantine'}</Button>
